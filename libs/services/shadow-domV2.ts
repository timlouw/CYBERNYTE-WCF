...
const __bind = (root: ShadowRoot, signal: { subscribe: (cb: (v: any) => void) => void }, id: string, update: (el: HTMLElement, v: any) => void): void => {
  const el = root.getElementById(id);
  if (el) signal.subscribe((v) => update(el, v));
};

/** Bind signal to element style property */
export const __bindStyle = (root: ShadowRoot, signal: { subscribe: (cb: (v: any) => void) => void }, id: string, prop: string): void => {
...
export const __bindAttr = (root: ShadowRoot, signal: { subscribe: (cb: (v: any) => void) => void }, id: string, attr: string): void => {
  __bind(root, signal, id, (el, v) => {
...
export const __bindText = (root: ShadowRoot, signal: { subscribe: (cb: (v: any) => void) => void }, id: string): void => {
  __bind(root, signal, id, (el, v) => {
    el.textContent = v;
  });
};
